...
from ui_mainwindow import Ui_MainWindow  # Импортируем сгенерированный интерфейс
...
from lab1_defs import (
    ...
    delete_national_name, delete_state_name, search_national_names_by_name, search_state_names_by_name,analyze
)
...
class MainWindow(QMainWindow):
    ...
    def update_gender_distribution(self):
        m_percentage_national, f_percentage_national = analyze(session, NationalNames)
        self.ui.label_m_percentage_national.setText(f"Мужчины (National): {m_percentage_national:.2f}%")
        self.ui.label_f_percentage_national.setText(f"Женщины (National): {f_percentage_national:.2f}%")
        m_percentage_state, f_percentage_state = analyze(session, StateNames)
        self.ui.label_m_percentage_state.setText(f"Мужчины (State): {m_percentage_state:.2f}%")
        self.ui.label_f_percentage_state.setText(f"Женщины (State): {f_percentage_state:.2f}%")
    ...
    def delete_state_name_by_id(self):
        try:
            ...
            QMessageBox.warning(self, "Ошибка", "Введите корректныйID")
    ...
    def f(self, item):
        ...
        id_value = text[id_start:id_end].strip()
